...
An identity is `{ "kind": "exactTitle", "value": "My document" }`. Restore prefers an exact title match; this is not a saved runtime window ID or a command to recreate content. If the title has changed, restore can reuse the sole app window already in its saved workspace, including when several saved entries target that same app/workspace. It does not require recreating the saved window count. A title match reserved for a different destination cannot be reused this way. Other zero/multiple matches stay unresolved; a user may choose a current window in the preview. Selections are tied to that snapshot and discarded when the preview changes.
