...
public struct RestoreReport: Codable, Sendable {
    ...
    public var allPlacementsVerified: Bool { !outcomes.isEmpty && outcomes.allSatisfy { $0.status == .completed } }
...
@MainActor public final class RestoreEngine {
    ...
    public func apply(_ preview: RestorePlan, safariConsent: SafariConsent? = nil,
                      resolutions: [WindowResolution] = []) async throws -> RestoreReport {
        ...
        return RestoreReport(profileName: preview.profile.name, outcomes: outcomes, cleanup: "Apps outside this profile were left open.")
    }
